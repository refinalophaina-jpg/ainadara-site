...
export function mountDevice(host, api) {
    ...
    <div class="device-stage">
      <div class="device-assembly" tabindex="0" aria-label="Pump controls. Use the side buttons and numeric keypad.">
        ...
        <section class="pcu" aria-label="Patient care unit simulator">
          ...
          <div class="pcu-lower">
            <div class="pcu-utilities"><button id="pcu-silence" class="hardware-key silence" title="Acknowledge the visual alarm. Audio is not simulated.">Silence</button><button id="pcu-options" class="hardware-key">Options</button><div class="power-light" aria-hidden="true"></div><span class="ac-label">AC power<br>Simulated</span></div>
      ...
      <div class="device-simulation-tools"><span id="device-clock">Simulated time 00:00:00</span><button id="device-startup" class="secondary">Rehearse startup</button><button id="device-advance" class="secondary">Advance 1 minute</button><button id="device-alarm" class="secondary">Introduce occlusion</button></div>
      <p class="device-caption">8015-inspired controls, generic labels. Press the physical keys beside or below the screen. Channel A only · visual alarms · no clinical use. Reference workflow: v12.1; your pump’s firmware is not yet verified. Startup rehearsal is optional.</p>
    </div>
  ...
  function back(){
    ...
    else if(['options','volume','infusionmenu','dataset','software','audio','profileconfirm','newpatient'].includes(view))go('overview');
  ...
  $('pcu-silence').addEventListener('click',()=>{silenced=true;render();});
  ...
  function render(){
    ...
    if(view==='standby'){
      ...
      row('Yes','Use this profile',()=>go('overview'));
      ...
      action(1,'CONFIRM',pendingArea?()=>{api.selectArea(pendingArea);go('overview');}:null);
      ...
      row('Calculated rate',`${api.format(result.rate)} mL/h`);
      ...
      row(e.name,`${p.dose} ${e.doseUnit}`);
      row('Flow rate',`${api.format(s.result.rate)} mL/h`);
      row('VTBI',`${p.vtbi} mL`);
      row('Check against order','Learner acknowledgment',()=>api.checkOrder());
      action(1,'EDIT',()=>{api.dispatch({type:'edit'});go('program');});
      action(4,'START',c.setupComplete?()=>{api.dispatch({type:'start'});go('status');}:null);
      footer=!c.setupComplete?'Complete the setup scene before START':s.reason?'Soft-limit override recorded. Confirm the program before starting.':'Compare every parameter with the exercise order, then press Start.';
      ...
      if(page===0){
        ...
        row('Audio','Visual-only simulator',()=>go('audio'));
      }else{
      ...
      title='SOFTWARE VERSIONS';row('Simulator','AinaDara prototype 0.2');row('Reference workflow','8015 user manual · v12.1');row('Your pump firmware','Not verified');row('No device connection','No firmware is installed here');
      action(1,'EXIT',()=>go('overview'));footer='>Training implementation · not manufacturer software';
    }else if(view==='audio'){
      title='AUDIO';row('Visual alarms only','No alarm audio is reproduced');row('Silence control','Acknowledges the visual alert');
      ...
      row('Rate',`${api.format(s.result.rate)} mL/h`);
      ...
      row(active?`${s.program.dose} ${e.doseUnit}`:'Primary infusion',active?`${api.format(s.result.rate)} mL/h`:'Library-based programming');
    ...
    put('module-rate',active?api.format(Math.round(rate*10)/10):'—');
