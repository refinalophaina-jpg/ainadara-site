import { UNITS, evaluate, validateLibrary, parseCSV, initialState, transition, limitText, softRange, entryLabel } from './engine.js';
...
let demo, library, imported = false, mode = 'guided', state = initialState(), candidate = null, validationRevision = 0, renderedEvents = -1;
let device = null;
...
function checkpoints() {
  ...
  const result = [
    { title:'Select the care-area profile', done:mode === 'free' ? !!selected : selected.id === s.entry, detail:mode === 'free' ? selected.area : 'Use the profile and concentration in the exercise.' },
    ...
    { title:mode === 'guided' ? 'Match and check the order' : 'Complete your program check', done:$('order-checked').checked && (mode === 'free' || matched), detail:mode === 'guided' ? 'Profile, concentration, dose/rate, volume and weight.' : 'Self-reported review; no exercise order is scored.' },
...
function render() {
  ...
  text('program-feedback',messages[state.status] || (state.reason ? `Soft limit overridden for this exercise: ${state.reason}. Review your program, then start.` : state.result?.message));
...
function wireEvents() {
  ...
  $('start').addEventListener('click',() => dispatch({type:state.status === 'running' ? 'pause' : state.status === 'paused' ? 'resume' : 'start'}));
  ...
  $('order-checked').addEventListener('change',render);
...
try {
  ...
  device = mountDevice($('device-host'),{
    context:() => ({state, entry:entry(), program:program(), library, imported, editable:editable(), setupComplete:!setupEnabled||setupReady(setupState), lineChecked:$('line-checked').checked}),
    ...
    checkOrder:() => { $('order-checked').checked = true; render(); },
  ...
  setupScene.render();
  device.render();
  if(new URL(location.href).searchParams.get('library')==='workplace'){
    try{
      const local=await fetch(new URL('./local-library.json',import.meta.url));
      if(!local.ok)throw new Error('Start the private workplace preview or import the CSV manually.');
      const checked=validateLibrary(await local.json());
      if(checked.errors.length)throw new Error(checked.errors.join(' '));
      applyLibrary(checked.library);
    }catch(error){$('active-library-warning').hidden=false;text('active-library-warning',`Workplace library not loaded: ${error.message} Demo library remains active.`);}
  }
