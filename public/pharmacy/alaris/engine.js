...
export function evaluate(entry, program) {
  ...
  const rate = dose * unit.factor * (unit.weight ? weight : 1) / entry.concentration;
  const duration = vtbi / rate * 3600;
  if (!Number.isFinite(rate) || !Number.isFinite(duration) || rate <= 0 || duration <= 0) return { kind: 'invalid', message: 'These values cannot produce a finite infusion. Check your entries.' };
  ...
  if (low || high) return { kind: 'hard', rate, duration, message: `${dose} ${entry.doseUnit} is ${low ? 'below' : 'above'} the hard limit (${low ? entry.hardMin : entry.hardMax} ${entry.doseUnit}). Reprogram to continue.` };
  if ((Number.isFinite(entry.softMin)&&dose < entry.softMin) || (Number.isFinite(entry.softMax)&&dose > entry.softMax)) return { kind: 'soft', rate, duration, message: `${dose} ${entry.doseUnit} is outside the soft range of ${softRange(entry)} ${entry.doseUnit}. Reprogram or record a reason to override in this exercise.` };
  return { kind: 'ok', rate, duration, message: `Within the supplied training limits. ${entry.limitNotice||'Check against the scenario order before starting.'}` };
}
export function initialState() { return { status:'editing', result:null, program:null, entry:null, delivered:0, elapsed:0, events:[], reason:'' }; }
...
export function transition(state, action) {
  ...
  if (action.type === 'edit' && ['editing','ready','soft','blocked'].includes(state.status)) return { ...state, status:'editing', result:null, reason:'', entry:null, program:null };
  ...
  if (action.type === 'start' && state.status === 'ready') return event({ ...state, status:'running' }, 'Started', `${state.entry.name}: ${state.program.dose} ${state.entry.doseUnit}; ${state.result.rate} mL/h; VTBI ${state.program.vtbi} mL.`);
